...
public class JRSerializer implements Serializable {
	...
	public JRSerializer(String surveyDefXml, String surveyInstanceXml,
			FormIndex index) {
	...
	private static void writeTreeRefToOutputStream(TreeReference treeRef,
	...
	public static TreeReference deserializeTreeRef(int localIndexString,
			String serialzedRef) {
	...
	private static void readStringIntoTreeReference(TreeReference treeRef,
			String serialzedTreeRef) {
		try {
			treeRef.readExternal(
					FileUtils.convertStringToDataInputStream(serialzedTreeRef),
	...
	public static SurveyModel deserializeSurveyModel(String serializedModel) {
		...
		TreeReference treeRef = deserializeTreeRef(serializer.localIndex,
				serializer.serializedTreeRef);
		FormIndex index = new FormIndex(serializer.localIndex, treeRef);
		return SurveyModel.resumeSurveyModel(serializer.surveyDefXml,
				serializer.surveyInstanceXml, index);
	}

	private static JRSerializer getJRSerializerFromSerializedModel(
	...
	public static String serializeSurveyModel(SurveyModel model) {
		JRSerializer serializer = new JRSerializer(
				model.getInitialFormDefXML(), model.getAnsweredXML(),
